...
- **版本号格式**：`v主.次.修订.YYMMDD`（例：`v2.2.19.260826`），主页页脚 + 404 + 终端开机页 + loader 统一同步
- **中英文改动**：改一处文案必须查所有出现位置（5 套皮肤 + 终端交互命令 + i18n 字典 + 中英两版日志/README），改完用 `_tools/` 里的 jsdom 脚本扫描确认英文模式无中文残留；**英文措辞统一**：本站源码 = Source Code（必须带 code，终端命令输出也不例外）
- **临时文件**：测试环境、备份文件（尤其含 token 的凭据备份）用完立即删除，不留缓存；自检脚本归 `_tools/`（node_modules 不入库）
